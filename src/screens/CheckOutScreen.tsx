...
export default function CheckOutScreen() {
    ...
    const initialize = async () => {
        try {
            ...
            if (!needsLocation) {
                // Location not required, ready to check out
    ...
    const handleCheckOut = async () => {
        ...
        if (locationIsRequired) {
            ...
            if (!isWithinRadius) {
                Alert.alert(
                    'Out of Range',
                    `You are ${formatDistance(distance!)} away from the office. You must be within ${officeLocation.radius}m to check out.`
                );
                return;
            }
        }
        ...
        try {
            ...
            if (errorData) {
                ...
                if (errorData.distance && errorData.required_distance) {
                    errorMessage = `You are ${errorData.distance.toFixed(2)}m away from the office. You must be within ${errorData.required_distance}m to check out.`;
                }
            }
            ...
            <View style={styles.mainCard}>
                ...
                {!isWithinRadius && (
                    <View style={styles.infoBox}>
                        <Text style={styles.infoIcon}>ℹ️</Text>
                        <Text style={styles.infoText}>
                            You need to be within {officeLocation.radius}m of the office to check out.
            ...
            <View style={styles.buttonContainer}>
                <TouchableOpacity
                    style={[
                        ...
                        (!isWithinRadius || isLoading) && styles.buttonDisabled,
                    ]}
                    onPress={handleCheckOut}
